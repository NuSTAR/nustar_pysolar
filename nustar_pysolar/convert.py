...
def to_solar(evtdata, hdr, **kwargs):
	""" Main script to convert the events to solar coordinates.
		...
		Inputs: evtdata and hdr from the input FITS file.

		Output: tbldata and outhdr that are a binary table and header
		to be written to the output FITS file.
...
def convert_file(infile, **kwargs):
	""" Wrapper to read the input file, convert the data to solar
		...
		usage: convert_file('path/to/file/nuXXXA06_cl.evt')
